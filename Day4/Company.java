...
public class Company {
    public static void main(String[] args) {
        ...
        ArrayList<Employee> _myEmployees = new ArrayList<>();
        ...
        boolean isExistEmployee;
        //Khai báo vòng lặp
        while (condition == true) {
            System.out.println("------------------------------------------");
            System.out.println("+++++++++Company Management v1.0+++++++++");
            ...
            switch (select) {
                        ...
                        if (isNum(_tempStr)) {
                            ...
                            if (_tempId == 0) {
                                ...
                                if (isExistDepartment)
                                {
                                    ...
                                    for(int i=0;i<_myEmployees.size();i++)
                                    {
                                        if(_myEmployees.get(i).getIdDepartment() == _tempId) {
                                            numberEmployee += 1;
                                            System.out.println( _myEmployees.get(i).ShowInfor());
                                        }
                                    }
                                    if(numberEmployee==0)
                                    {
                                        System.out.println("This department doesn't have any employee!");
                                    }
                                } else {
                ...
                case "3":
                        ...
                        if (isNum(_tempStr)) {
                            ...
                            if (_tempId == 0) {
                                ...
                                if (isExistDepartment) {
                                    ...
                                    int sizeEmployee = _myEmployees.size();
                                    for(int i = sizeEmployee-1;i>=0;i--){
                                        if (_myEmployees.get(i).getIdDepartment() == _tempId) {
                                            _myEmployees.remove(i);
                                        }
                                    }
                                    System.out.println("Delete all employee in this department!");
                    ...
                    break;
                case "4":
                    System.out.println("Input Id of New Employee : ");
                    _tempStr = s.nextLine();
                    if (isNum(_tempStr)) {
                        _tempId = Integer.parseInt(_tempStr);
                        isExistEmployee = false;
                        for (Employee _employee : _myEmployees) {
                            if (_employee.getId() == _tempId) {
                                isExistEmployee = true;
                                break;
                            }
                        }
                        if (isExistEmployee) {
                            System.out.println("ID Employee " + _tempId + " is exist!");
                        } else {
                            System.out.println("Input Name of New Employee : ");
                            _tempName = s.nextLine();
                            System.out.println("Input Description of New Employee : ");
                            _tempDes = s.nextLine();

                            System.out.println("Input ID Department of this employee : ");
                            _tempStr = s.nextLine();

                            isExistDepartment=false;
                            for (Department _department:_myDepartments) {
                                if(_department.getId() == Integer.parseInt( _tempStr)) isExistDepartment=true;
                            }
                            if(isExistDepartment) {
                                _myEmployees.add(new Employee(_tempId, _tempName, _tempDes,Integer.parseInt( _tempStr)));
                                System.out.println("Add Employee " + _tempName + " success!");
                            }else{
                                System.out.println("ID Department isn't exist!");
                            }
                        }
                    } else {
                        System.out.println("Wrong format!");
...
//                        for (Employee _employee : _myEmployees) {
//                            if (_employee.getId() == _tempId) {
//                                System.out.println( _employee.ShowInfor() +" . Department: " + _employee.getIdDepartment());
//                                isExistEmployee = true;
//                                break;
//                            }
//                        }
//                        if (!isExistEmployee) {
//                            System.out.println("Not found Employee with ID " + _tempId);
//                        }
//                    } else {
//                        System.out.println("Wrong format!");
//                    }

                    if(_myEmployees.size()==0)
                    {
                        System.out.println("0 Employee!");
                    }else{
                        for(int i=0;i<_myEmployees.size();i++)
                            System.out.println( _myEmployees.get(i).ShowInfor() +" . Department: " +  _myEmployees.get(i).getIdDepartment());
                    }

                    break;
                case "6"://Delete Infor Employee
                    System.out.println("Input Id of New Employee : ");
                    _tempStr = s.nextLine();
                    if (isNum(_tempStr)) {
                        _tempId = Integer.parseInt(_tempStr);
                        ...
                        if (!isExistEmployee) {
                            ...
                            System.out.println("Delete Employee with ID " + _tempId+ " success!");
                        }
                    } else {
                        System.out.println("Wrong format!");
                    }
...
class Employee {
    ...
    public Employee(int id, String name, String description,int idDepartment) {
        setId(id);
        setName(name);
